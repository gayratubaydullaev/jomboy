...
RLS: migrations dan keyin ixtiyoriy — `apps/api/prisma/apply-rls.sh` (yoki `psql $DATABASE_URL -f apps/api/prisma/rls-policies.sql`). Jadval `orders` (oldingi `"Order"`) uchun siyosatalar moslashtirilgan. Production da avval backup oling.
...
- Matnlar JSON da: [`apps/web/messages/uz.json`](apps/web/messages/uz.json) va [`apps/web/messages/ru.json`](apps/web/messages/ru.json) — kalitlar bir xil boʻlishi kerak.
- Tanlangan til cookie da saqlanadi: `myshop_locale` (`uz` yoki `ru`). Standart til: `uz`.
- Komponentlarda: `useTranslation()` → `t('header.catalog')` va hokazo; `Intl` uchun `intlLocale` (`uz-UZ` / `ru-RU`).
- Yangi til qoʻshish: yangi `apps/web/messages/<kod>.json` faylini `uz.json` dan nusxalab tarjima qiling; [`apps/web/src/i18n/config.ts`](apps/web/src/i18n/config.ts) da `SUPPORTED_LOCALES`, `parseLocale`, `htmlLang`, `intlLocaleTag` ni yangilang; [`apps/web/src/i18n/dictionaries.ts`](apps/web/src/i18n/dictionaries.ts) da import va `messagesByLocale` obyektiga qoʻshing; [`LanguageSwitcher`](apps/web/src/components/layout/language-switcher.tsx) da variant qoʻshing.
