...
import type { ApiLocale } from '../i18n/api-locale';
...
export class AuthService implements OnModuleInit {
  ...
  async logout(refreshToken: string) {
    await this.prisma.refreshToken.deleteMany({ where: { token: hashRefreshToken(refreshToken) } });
    return { success: true };
  }
  ...
  async sendOtp(email: string, locale: ApiLocale = 'uz'): Promise<{ success: boolean }> {
    ...
    await this.mailer.sendMail({
      to: email,
      subject: apiT(locale, 'email.otpSubject'),
      text: apiT(locale, 'email.otpText', { code }),
      html: apiT(locale, 'email.otpHtml', { code }),
    });
    return { success: true };
  }
  ...
  async registerOrLoginGuest(data: {
    ...
    if (byEmail) {
      ...
      if (!byEmail.isGuest) throw new ForbiddenException('Guest verification required');
      return this.login(byEmail);
    }
    const user = await this.prisma.user.create({
      data: {
        email,
