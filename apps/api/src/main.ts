...
async function bootstrap() {
  ...
  app.use(
    '/uploads',
    express.static(uploadsDir, { index: false, dotfiles: 'deny', fallthrough: false }),
  );
  ...
  app.enableCors({
    ...
    credentials: true,
    allowedHeaders: ['Content-Type', 'Authorization', 'x-cart-session', 'x-csrf-token', 'x-locale', 'Accept'],
  });
  ...
  app.useGlobalPipes(
    new ValidationPipe({
      ...
      exceptionFactory: (errors: ValidationError[]) => {
        const flatten = (errs: ValidationError[], prefix = ''): { field: string; messages: string[] }[] => {
          const out: { field: string; messages: string[] }[] = [];
          for (const e of errs) {
            const field = prefix ? `${prefix}.${e.property}` : e.property;
            if (e.constraints) {
              out.push({ field, messages: Object.values(e.constraints) });
            }
        ...
        return new BadRequestException({
          message: apiT(getApiLocale(), 'errors.validation'),
