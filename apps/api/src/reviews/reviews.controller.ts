...
export class ReviewsController {
  ...
  private resolveUserId(req: Request): string | undefined {
    ...
    try {
      const payload = this.jwtService.verify(auth.slice(7)) as { sub?: string };
      return payload.sub;
    } catch {
      return undefined;
    }
  ...
  @Public()
  @ApiOperation({ summary: 'Check if current buyer can leave a review (optional JWT)' })
  canReview(@Param('productId') productId: string, @Req() req: Request) {
    const userId = this.resolveUserId(req);
    return this.reviews.canLeaveReviewForUser(productId, userId);
  }
