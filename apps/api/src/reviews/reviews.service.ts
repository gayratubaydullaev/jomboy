...
export class ReviewsService {
  ...
  /** Guests and non-buyers get canReview: false without HTTP 403. */
  async canLeaveReviewForUser(
    productId: string,
    userId?: string,
  ): Promise<{ canReview: boolean; purchaseCount: number; reviewCount: number }> {
    const empty = { canReview: false, purchaseCount: 0, reviewCount: 0 };
    if (!userId) return empty;
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { role: true },
    });
    if (!user || user.role !== UserRole.BUYER) return empty;
    return this.canLeaveReview(productId, userId);
  }
