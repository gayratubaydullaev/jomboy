...
- **Маршрут `/telegram-app`:** точка входа Mini App: свой layout с `TelegramWebAppInit` (жесты, повторный init при фокусе/навигации). Hub (`/telegram-app`) перенаправляет на **`/telegram-app/catalog`**.
- **Полноценная витрина в TWA:** `/telegram-app/catalog`, `/telegram-app/product/[id]`, `/telegram-app/cart`, `/telegram-app/checkout`, `/telegram-app/success` — каталог, карточка, корзина и оплата без выхода в браузер. В BotFather укажите Menu Button URL: `https://<ваш-домен>/telegram-app/catalog`.
