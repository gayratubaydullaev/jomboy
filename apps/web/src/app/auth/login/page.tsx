...
function LoginForm() {
  ...
  const startTelegramLogin = () => {
    ...
    apiFetch(API_URL + '/auth/telegram/request-login', { method: 'POST' })
      .then(async (r) => {
        ...
        if (!r.ok) {
          setTgError(data?.message ?? t('auth.login.telegramNotConfiguredHint'));
          setTgLoading(false);
          return;
        }
        if (!data.token || !data.loginUrl) {
          setTgError(data?.message ?? t('auth.login.telegramNotConfigured'));
          setTgLoading(false);
          return;
        }
      ...
      .catch(() => {
        setTgError(t('auth.login.serverError'));
        setTgLoading(false);
      });
  };
  ...
  const submit = handleSubmit((data) => {
    ...
    apiFetch(API_URL + '/auth/login', {
      ...
      .catch((err) => {
        const msg = err?.message ?? err?.error ?? t('auth.login.invalidCredentials');
        setSubmitError(msg === 'Invalid credentials' ? t('auth.login.invalidCredentials') : msg);
      })
  ...
  const reasonMessage =
    reason === 'favorites'
      ? t('auth.login.reasonFavorites')
      : next === '/checkout'
        ? t('auth.login.reasonCheckout')
        : null;
    ...
    <div className="w-full max-w-md mx-auto px-0 sm:px-4 md:px-6 py-8 sm:py-12">
      <Card className="border-border/80 shadow-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription>
            {reasonMessage ?? t('auth.login.description')}
          </CardDescription>
          <div className="flex flex-col gap-2 w-full mt-4" aria-label={t('auth.login.telegramButton')}>
            <span className="text-xs font-medium text-muted-foreground">{t('auth.login.telegramSection')}</span>
            <Button
              type="button"
              variant="outline"
          ...
          <div className="relative flex items-center gap-2 w-full pt-2 mt-2">
            ...
            <span className="text-xs text-muted-foreground">{t('auth.login.dividerEmail')}</span>
            <div className="flex-1 h-px bg-border" />
          </div>
        </CardHeader>
        <form onSubmit={submit}>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="login-email">{t('auth.login.labelEmail')}</Label>
              <Input
                id="login-email"
                type="email"
            ...
            <div className="space-y-2">
              <Label htmlFor="login-password">{t('auth.login.labelPassword')}</Label>
