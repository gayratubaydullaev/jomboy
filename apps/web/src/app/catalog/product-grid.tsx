...
export function ProductGrid({ linkPrefix = '' }: { linkPrefix?: string }) {
  ...
  const products = (data?.data ?? []).map(apiProductToCardProduct);
  ...
  if (error) {
    return (
      <div className="text-center py-16 md:py-24 bg-muted/30 rounded-2xl border border-dashed space-y-4">
        <p className="text-muted-foreground text-lg">{t('catalog.loadError')}</p>
    ...
    return (
      <div className="text-center py-16 md:py-24 bg-muted/30 rounded-2xl border border-dashed">
        <p className="text-muted-foreground mb-4 text-lg">
          {hadSearch ? t('catalog.emptySearch') : t('catalog.emptyNoProducts')}
        </p>
        <Button asChild>
          <Link href="/catalog">{t('catalog.goToCatalog')}</Link>
        </Button>
      </div>
    );
  }
