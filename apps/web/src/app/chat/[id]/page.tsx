...
export default function ChatRoomPage() {
  ...
  const send = (e: React.FormEvent) => {
    ...
    apiFetch(`${API_URL}/chat/sessions/${id}/messages`, {
      ...
      .then(async (r) => {
        ...
        if (!r.ok) throw new Error(typeof data?.message === 'string' ? data.message : t('chat.sendError'));
  ...
  const headerTitle = otherUser
    ? `${otherUser.firstName} ${otherUser.lastName}`.trim() || (isSeller ? t('chat.otherBuyer') : t('chat.otherSeller'))
    : t('chat.roomFallback');
  const headerRoleLabel = otherUser ? (isSeller ? t('chat.otherBuyer') : t('chat.otherSeller')) : null;

  const inputPlaceholder = sellerChatDisabled
    ? session?.chatWithSellerEnabled === false
      ? t('chat.placeholderAdminOff')
      : t('chat.placeholderSellerOff')
    : t('chat.placeholderWrite');
        ...
        <div className="flex-1 min-w-0">
          ...
          {session?.product && (
            <Link href={`/product/${session.product.id}`} className="text-xs text-muted-foreground truncate block hover:text-primary hover:underline">
              {isSeller && <span className="text-muted-foreground/80">{t('chat.productPrefix')} </span>}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {sellerChatDisabled && (
          <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-3 text-sm text-amber-800 dark:text-amber-200">
            {t('chat.sellerPausedNew')}
          </div>
        )}
        {(!messages || messages.length === 0) && !sellerChatDisabled && (
          <p className="text-center text-muted-foreground text-sm py-8">{t('chat.noMessagesYet')}</p>
        )}
