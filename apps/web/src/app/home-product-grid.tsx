...
export function HomeProductGrid() {
  ...
  if (loadError) {
    return (
      <div className="text-center py-16 md:py-24 bg-muted/30 rounded-2xl border border-dashed space-y-4">
        <p className="text-muted-foreground text-lg">{t('home.loadError')}</p>
        <Button type="button" onClick={() => { setLoading(true); loadPage(1, false); }}>
          {t('common.retry')}
        </Button>
      </div>
    );
  ...
  if (!products.length) {
    return (
      <div className="text-center py-16 md:py-24 bg-muted/30 rounded-2xl border border-dashed">
        <p className="text-muted-foreground mb-4 text-lg">{t('home.emptyTitle')}</p>
        <Button asChild>
          <Link href="/catalog">{t('home.goToCatalog')}</Link>
        </Button>
      </div>
    );
  }
