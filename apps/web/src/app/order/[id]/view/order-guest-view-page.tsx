...
export default function OrderGuestViewPage() {
  ...
  if (error) {
    return (
      <div className="max-w-lg mx-auto px-0 sm:px-4 md:px-6 py-8 text-center space-y-4">
        <h1 className="text-xl font-bold">{t('orderGuestView.notFoundTitle')}</h1>
        <p className="text-muted-foreground text-sm">{t('orderGuestView.notFoundHint')}</p>
        <Button asChild>
          <Link href="/auth/login?next=/orders">{t('orderGuestView.login')}</Link>
        </Button>
        <Button asChild variant="outline">
          <Link href="/">{t('orderGuestView.home')}</Link>
        </Button>
      </div>
    );
  }
  ...
  const shopName = order.seller?.shop?.name ?? t('orderGuestView.shopDefault');
    ...
    <div className="max-w-2xl mx-auto px-0 sm:px-4 md:px-6 py-8 space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        <Link href="/" className="text-muted-foreground hover:text-foreground text-sm">
          {t('orderGuestView.backHome')}
        </Link>
        <h1 className="text-xl sm:text-2xl font-bold">{t('orderGuestView.title', { number: order.orderNumber })}</h1>
      </div>
      <Card>
        <CardHeader className="pb-2">
          ...
          {isPickup && pickupAddress && (
            <p className="text-sm text-muted-foreground mt-1">
              {t('orderGuestView.pickupAddress')} {formatPickupAddress(pickupAddress) || t('orderGuestView.addressUnknown')}
            </p>
          )}
          {isPickup && !pickupAddress && <p className="text-sm text-muted-foreground mt-1">{t('orderGuestView.pickupContact')}</p>}
        </CardHeader>
        <CardContent className="space-y-3">
          <ul className="space-y-2">
            {order.items?.map((item) => (
              <li key={item.id} className="flex gap-3 items-center text-sm">
                ...
                <span className="flex-1 truncate">{item.product?.title ?? t('orderGuestView.productDefault')}</span>
          ...
          <p className="font-semibold pt-1 border-t">
            {t('orderGuestView.total')} {formatPrice(Number(order.totalAmount))} {cur}
          </p>
        </CardContent>
      </Card>
      <div className="flex flex-wrap gap-3">
        <Button asChild>
          <Link href="/catalog">{t('orderGuestView.catalog')}</Link>
        </Button>
        <Button asChild variant="outline">
          <Link href="/">{t('orderGuestView.home')}</Link>
        </Button>
