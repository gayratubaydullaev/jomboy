...
export function ProductSpecsModal({ product, trigger }: ProductSpecsModalProps) {
  ...
  const baseRows = [
    { name: t('product.specCategory'), value: product.category?.name || ns },
    { name: 'SKU', value: product.sku || product.id.slice(0, 8) },
    { name: t('product.sellerRow'), value: product.shop?.name || ns },
  ];
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('product.specsModalTitle')}</DialogTitle>
          <DialogDescription>{product.title ?? ''}</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <h3 className="font-semibold mb-2 text-lg">{t('product.specsBlockTitle')}</h3>
          ...
          {product.description && (
            <div>
              <h3 className="font-semibold mb-2 text-lg">{t('product.descriptionHeading')}</h3>
