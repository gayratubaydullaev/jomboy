...
export default function SellerDashboardPage() {
  ...
  return (
    <div className="min-w-0 space-y-8">
      <DashboardPageHeader
        eyebrow={t('seller.home.eyebrow')}
        subtitle={stats?.shopName?.trim() || undefined}
        title={t('seller.home.title')}
        description={t('seller.home.description')}
      >
        <Button asChild>
              ...
              <Card className="h-full transition-colors hover:bg-accent/50 hover:border-primary/30">
                <CardContent className="p-5 flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">{t('seller.home.cardProducts')}</p>
                  </div>
                </CardContent>
              </Card>
            </Link>
            ...
            <Card className="h-full transition-colors hover:bg-accent/50 hover:border-primary/30">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">{t('seller.home.cardOrders')}</p>
                  {stats.pendingOrdersCount != null && stats.pendingOrdersCount > 0 && (
                    <p className="text-xs text-amber-600 dark:text-amber-400 font-medium mt-0.5">
                      {t('seller.home.pendingConfirm', { count: stats.pendingOrdersCount })}
                    </p>
                  )}
                </div>
              </CardContent>
          ...
          <Link href="/seller/stats">
            <Card className="h-full transition-colors hover:bg-accent/50 hover:border-primary/30">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-xl font-bold truncate" title={formatPrice(Number(stats.totalRevenue))}>
                    {formatPrice(Number(stats.totalRevenue))} {currency}
                  </p>
                  <p className="text-sm text-muted-foreground">{t('seller.home.cardRevenueHint')}</p>
                  {typeof stats.balance === 'number' && stats.balance > 0 && (
                    <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                      {t('seller.home.balanceHint', { amount: formatPrice(stats.balance), currency })}
                    </p>
                  )}
                </div>
              </CardContent>
          ...
          <Link href="/seller/settings">
            <Card className="h-full transition-colors hover:bg-accent/50 hover:border-primary/30">
              <CardContent className="p-5 flex items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="font-semibold">{t('seller.home.cardSettings')}</p>
                  <p className="text-sm text-muted-foreground">{t('seller.home.cardSettingsHint')}</p>
                </div>
              </CardContent>
            </Card>
          </Link>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-base">{t('seller.home.recentOrders')}</CardTitle>
          <Button variant="ghost" size="sm" asChild>
            <Link href="/seller/orders">
              {t('seller.home.seeAll')} <ArrowRight className="h-4 w-4 ml-1" />
            </Link>
          </Button>
        </CardHeader>
        <CardContent>
          {recentOrders === null ? (
            ...
            <p className="text-sm text-muted-foreground py-4">
              {t('seller.home.noOrdersLine')}{' '}
              <Link href="/seller/products" className="text-primary hover:underline">
                {t('seller.home.noOrdersLink')}
              </Link>{' '}
              {t('seller.home.noOrdersPromote')}
            </p>
          ) : (
                  ...
                  <Link href="/seller/orders" className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-lg border bg-muted/30 hover:bg-muted/50 transition-colors">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
                        {orderStatusLabel(o.status, o.deliveryType, t)}
                      </span>
                    </div>
                    <span className="font-semibold text-sm">
                      {formatPrice(Number(o.totalAmount))} {currency}
                    </span>
                  </Link>
                </li>
              ))}
