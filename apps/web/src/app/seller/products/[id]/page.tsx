...
export default function EditProductPage() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!categoryId) {
      toast.error(F('toastCategoryRequiredEdit'));
      return;
    }
    if (!imageUrls.length) {
      toast.error(F('toastImageRequiredEdit'));
      return;
    }
    ...
    apiFetch(`${API_URL}/products/${id}`, {
      ...
      .then((r) => r.json())
      .then(() => {
        toast.success(F('toastProductSaved'));
        router.push('/seller/products');
      })
      .catch((err) => {
        const msg = err?.message ?? err?.response?.data?.message ?? (typeof err === 'string' ? err : F('toastProductSaveFailed'));
        toast.error(msg);
      })
  ...
  if (product === undefined) {
    return (
      <div className="mx-auto min-w-0 max-w-2xl space-y-6">
        <DashboardPageHeader eyebrow={F('pageEyebrow')} title={F('editTitle')} description={F('editLoadingDesc')}>
  ...
  if (product === null) {
    return (
      <div className="mx-auto min-w-0 max-w-2xl space-y-6">
        <DashboardPageHeader eyebrow={F('pageEyebrow')} title={F('editTitle')} description={F('editNotFoundDesc')}>
          <Button variant="outline" size="sm" className="min-h-[40px] touch-manipulation" asChild>
            <Link href="/seller/products">
              <ArrowLeft className="mr-2 h-4 w-4" />
              {F('backToList')}
        ...
        <DashboardEmptyState icon={PackageX} title={F('editNotFoundTitle')} description={F('editWrongShopDesc')}>
          <Button asChild>
            <Link href="/seller/products">{F('linkProductList')}</Link>
          </Button>
        </DashboardEmptyState>
      </div>
    );
  ...
  return (
    <div className="mx-auto min-w-0 max-w-2xl space-y-6">
      <DashboardPageHeader
        eyebrow={F('pageEyebrow')}
        title={F('editTitle')}
        subtitle={product.title}
        description={F('editDescription')}
      >
        <Button variant="outline" size="sm" className="min-h-[40px] touch-manipulation" asChild>
          <Link href="/seller/products">
            <ArrowLeft className="mr-2 h-4 w-4" />
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder={F('phTitle')} className="mt-1" required />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={unit} onChange={(e) => setUnit(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
            ...
            <div className="flex flex-wrap gap-2 items-start">
              {imageUrls.map((url, i) => (
                <div key={i} className="relative group w-24 h-24 rounded-lg border overflow-hidden bg-muted shrink-0">
                  ...
                  {i === 0 && (
                    <span className="absolute bottom-0 left-0 right-0 bg-primary/90 text-primary-foreground text-[10px] text-center py-0.5">{F('badgeMainImage')}</span>
                  )}
                </div>
              ))}
              <label className="w-24 h-24 rounded-lg border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-muted/50 transition shrink-0">
                <input type="file" accept="image/*" className="hidden" multiple onChange={uploadMultipleImages} disabled={uploading} />
                {uploading ? (
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input type="text" inputMode="numeric" value={price} onChange={(e) => setPrice(e.target.value)} placeholder={F('phPrice')} className="mt-1" />
              </div>
              <div>
                ...
                <Input type="text" inputMode="numeric" value={comparePrice} onChange={(e) => setComparePrice(e.target.value)} placeholder={F('phCompare')} className="mt-1" />
              </div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm font-medium">{F('labelActive')}</span>
            </label>
          </CardContent>
        </Card>
