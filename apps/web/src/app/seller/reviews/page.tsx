...
export default function SellerReviewsPage() {
  ...
  const handleReply = (reviewId: string) => {
    ...
    apiFetch(`${API_URL}/reviews/${reviewId}/reply`, {
      ...
      .then(() => {
        toast.success(t('seller.reviews.toastReplySent'));
        setDrafts((prev) => Object.fromEntries(Object.entries(prev).filter(([k]) => k !== reviewId)));
        setReplyingId(null);
        load();
      })
      .catch(() => {
        toast.error(t('seller.reviews.toastReplyError'));
        setReplyingId(null);
      });
  };
  ...
  return (
    <div className="min-w-0 max-w-full space-y-6">
      <DashboardPageHeader
        eyebrow={t('seller.reviews.eyebrow')}
        title={t('seller.reviews.title')}
        description={t('seller.reviews.description')}
      />
      ...
      <Card>
        <CardHeader className="border-b border-border/60 pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {t('seller.reviews.cardTitle', { count: reviews.length })}
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-5">
          {reviews.length === 0 ? (
            <DashboardEmptyState
              icon={MessageSquare}
              title={t('seller.reviews.emptyTitle')}
              description={t('seller.reviews.emptyDescription')}
            />
          ) : (
                ...
                <li key={r.id} className="p-4 rounded-lg border bg-card space-y-3">
                  ...
                  {r.sellerReply ? (
                    <div className="text-sm pl-3 border-l-2 border-primary/30 text-muted-foreground bg-muted/50 rounded-r py-2 px-3">
                      <span className="font-medium text-foreground">{t('seller.reviews.yourReply')}</span> {r.sellerReply}
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <textarea
                        ...
                        maxLength={1000}
                      />
                      <Button size="sm" onClick={() => handleReply(r.id)} disabled={!(drafts[r.id] ?? '').trim() || replyingId === r.id}>
                        {replyingId === r.id ? t('seller.reviews.submitting') : t('seller.reviews.submitReply')}
                      </Button>
                    </div>
                  )}
                </li>
