...
const mainItems: { href: string; labelKey: string; icon: typeof LayoutDashboard; moderatorPermission?: ModeratorPermissionKey }[] = [
  { href: '/admin', labelKey: 'home', icon: LayoutDashboard },
  { href: '/admin/notifications', labelKey: 'notifications', icon: Bell },
  { href: '/admin/users', labelKey: 'users', icon: Users },
  { href: '/admin/sellers', labelKey: 'sellers', icon: Store },
  { href: '/admin/seller-applications', labelKey: 'sellerApplications', icon: FileCheck, moderatorPermission: 'canApproveSellerApplications' },
  { href: '/admin/pending-shop-updates', labelKey: 'pendingShopUpdates', icon: FileEdit, moderatorPermission: 'canApproveShopUpdates' },
  { href: '/admin/orders', labelKey: 'orders', icon: ShoppingBag },
  { href: '/admin/products', labelKey: 'productsModeration', icon: Package, moderatorPermission: 'canModerateProducts' },
  { href: '/admin/reviews', labelKey: 'reviews', icon: MessageSquare, moderatorPermission: 'canModerateReviews' },
  { href: '/admin/stats', labelKey: 'stats', icon: BarChart3 },
  { href: '/admin/payouts', labelKey: 'payouts', icon: Banknote },
];
const secondaryItems: { href: string; labelKey: string; icon: typeof FolderTree; adminOnly?: boolean }[] = [
  { href: '/admin/categories', labelKey: 'categories', icon: FolderTree, adminOnly: true },
  { href: '/admin/banners', labelKey: 'banners', icon: ImageIcon, adminOnly: true },
...
function NavContent({
  ...
  return (
    <>
      ...
      <p id="nav-main-label" className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
      ...
      <div className="flex flex-col gap-0.5" aria-labelledby="nav-main-label">
        {filteredMain.map((item) => (
          <NavLink
            ...
            pathname={pathname}
            href={item.href}
            label={t(`admin.nav.${item.labelKey}`)}
            icon={item.icon}
            onClick={onNavClick}
          />
        ))}
      ...
      {filteredSecondary.length > 0 && (
        <>
          <div className="h-px bg-border/60 my-2" aria-hidden />
          <p id="nav-secondary-label" className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
            {t('admin.nav.catalogSettings')}
          </p>
    ...
    <>
      ...
      >
        ...
        <Link href="/admin" className="flex items-center gap-2 min-h-[44px] items-center" onClick={() => setMobileOpen(false)}>
          ...
          <span className="font-semibold text-sm">{t('admin.nav.panelTitle')}</span>
        </Link>
        <LanguageSwitcher className="h-9 w-[6.5rem] text-xs shrink-0" />
      </div>
      ...
      <aside
        ...
        aria-label={t('admin.nav.panelAria')}
      >
        <div className="flex items-center justify-between gap-2 border-b border-border/60 bg-card/80 p-3 backdrop-blur-sm md:p-4">
          ...
          >
            ...
            <span className="truncate text-sm font-semibold">{t('admin.nav.panelTitle')}</span>
          </Link>
        ...
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto overscroll-contain p-3 md:p-4" aria-label={t('admin.nav.mainMenuAria')}>
          <NavContent pathname={pathname} onNavClick={() => setMobileOpen(false)} adminUser={adminUser} t={t} />
        </nav>
        <DashboardNavFooter />
      </aside>
    </>
