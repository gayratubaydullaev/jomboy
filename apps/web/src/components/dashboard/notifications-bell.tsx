...
export function NotificationsBell({ basePath, className }: NotificationsBellProps) {
  ...
  const formatTime = (iso: string) => {
    ...
    if (diff < 1) return t('chat.timeJustNow');
    if (diff < 60) return t('chat.timeMins', { n: Math.floor(diff) });
    if (diff < 1440) return t('chat.timeHours', { n: Math.floor(diff / 60) });
  ...
  return (
    <div ref={containerRef} className={cn('relative', className)}>
      <button
        ...
        aria-label={unreadCount > 0 ? t('dashboard.notifications.bellAriaUnread', { count: unreadCount }) : t('dashboard.notifications.bellAria')}
        aria-expanded={open}
      >
        ...
        <div className="absolute top-full right-0 mt-1 z-50 w-[min(320px,calc(100vw-2rem))] bg-card border border-border rounded-xl shadow-xl overflow-hidden flex flex-col max-h-[80vh]">
          ...
          <div className="overflow-y-auto flex-1 min-h-0">
            {loading ? (
              ...
              <p className="p-4 text-sm text-muted-foreground text-center">{t('dashboard.notifications.empty')}</p>
            ) : (
              <ul className="py-1">
                {list.map((n) => (
                  <li key={n.id}>
                    <Link
                      href={n.link && isSafeInternalPath(n.link) ? n.link : basePath}
                      onClick={() => {
          ...
          >
            {t('dashboard.notifications.seeAll')} <ChevronRight className="h-4 w-4" />
          </Link>
        </div>
      )}
    </div>
