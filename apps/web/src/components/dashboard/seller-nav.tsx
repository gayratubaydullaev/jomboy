...
function NavContent({
  ...
  onNavClick?: () => void;
  mainItems: { href: string; label: string; icon: typeof LayoutDashboard; badgeKey?: 'pendingOrdersCount' }[];
  secondaryItems: { href: string; label: string; icon: typeof Settings }[];
  t: TranslateFn;
}) {
  return (
    <>
      <Link
        ...
        aria-label={t('seller.nav.backToSite')}
      >
      ...
      {shopSlug && (
        <Link
          ...
          aria-label={t('seller.nav.viewShop')}
        >
          <ExternalLink className="h-5 w-5 md:h-4 md:w-4 shrink-0" aria-hidden />
          <span>{t('seller.nav.viewShop')}</span>
        </Link>
      )}
      <div className="h-px bg-border/60 my-2" aria-hidden />
      <p id="seller-nav-main-label" className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
        {t('seller.nav.management')}
      </p>
      ...
      <Link
        ...
        aria-label={t('seller.nav.messages')}
      >
        <MessageCircle className="h-5 w-5 md:h-4 md:w-4 shrink-0" aria-hidden />
        <span>{t('seller.nav.messages')}</span>
      </Link>
      <div className="h-px bg-border/60 my-2" aria-hidden />
      <p id="seller-nav-settings-label" className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
        {t('seller.nav.settingsSection')}
      </p>
...
export function SellerNav() {
  ...
  const mainItems = [
    { href: '/seller', label: t('seller.nav.home'), icon: LayoutDashboard },
    { href: '/seller/notifications', label: t('seller.nav.notifications'), icon: Bell },
    { href: '/seller/products', label: t('seller.nav.products'), icon: Package },
    { href: '/seller/orders', label: t('seller.nav.orders'), icon: ShoppingBag, badgeKey: 'pendingOrdersCount' as const },
    { href: '/seller/reviews', label: t('seller.nav.reviews'), icon: Star },
    { href: '/seller/stats', label: t('seller.nav.stats'), icon: BarChart3 },
  ];
    ...
    <>
      ...
      >
        <button
          ...
          aria-label={t('seller.nav.openMenu')}
        >
      ...
      <aside
        ...
        aria-label={t('seller.nav.navAria')}
      >
        <div className="flex flex-col gap-3 border-b border-border/60 bg-card/80 p-3 backdrop-blur-sm md:p-4">
          <div className="flex items-start justify-between gap-2">
            <Link
              ...
              aria-label={t('seller.nav.homeAria')}
              onClick={() => setMobileOpen(false)}
            >
              <span className="flex items-center gap-2 text-sm font-semibold">
                <Store className="h-5 w-5 shrink-0 text-primary" aria-hidden />
                <span className="truncate">{t('seller.nav.panelTitle')}</span>
              </span>
              {shopName ? <span className="truncate pl-7 text-xs text-muted-foreground">{shopName}</span> : null}
            </Link>
          ...
          <Link
            ...
            aria-label={t('seller.nav.addProduct')}
          >
            <Plus className="h-4 w-4 shrink-0" />
            {t('seller.nav.addProductShort')}
          </Link>
        </div>
        <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto overscroll-contain p-3 md:p-4" aria-label={t('seller.nav.mainMenuAria')}>
          <NavContent
            pathname={pathname}
