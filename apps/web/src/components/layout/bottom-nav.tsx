...
export function BottomNav() {
  ...
  const navItems = useMemo(
    () => [
      ...
      { href: '/cart', label: t('nav.cart'), icon: ShoppingCart, badge: 'cart' as const },
      { href: '/favorites', label: t('nav.favorites'), icon: Heart, badge: 'favorites' as const },
      { href: '/account', hrefAlt: '/auth/login', label: t('nav.profile'), icon: User },
    ],
    [t],
  );
        ...
        >
          <div className="flex items-center gap-2 shrink-0 border-b border-border px-4 py-3">
            ...
            <h2 className="flex-1 font-semibold text-lg">
              {selectedCategory ? selectedCategory.name : t('nav.catalog')}
            </h2>
            <button
              type="button"
              onClick={closeCatalog}
          ...
          >
            {!selectedCategory ? (
              <>
                ...
                {categories.length === 0 && (
                  <div className="px-4 py-3 text-sm text-muted-foreground">{t('header.categoriesLoading')}</div>
                )}
                {categories.map((cat) => {
              ...
              <div className="py-2">
                {(selectedCategory.children ?? []).length === 0 ? (
                  <p className="px-4 py-3 text-sm text-muted-foreground">{t('header.noSubcategories')}</p>
                ) : (
