...
interface ProductCardProps {
  ...
  /** Prefix for in-app routes, e.g. `/telegram-app` */
  linkPrefix?: string;
  className?: string;
...
function ProductCardInner({
  ...
  return (
    ...
    >
      <Link href={`${linkPrefix}/product/${product.id}`} className="flex flex-col flex-1 min-h-0">
        ...
        <div className="p-3 flex flex-col flex-1 gap-0">
          ...
          <div className="flex flex-col">
            {comparePrice != null && comparePrice > price && (
              <span className="text-sm text-muted-foreground line-through leading-tight">
                {formatPrice(comparePrice)} {t('checkout.currency')}{product.unit ? ` / ${product.unit}` : ''}
              </span>
            )}
            <div className="flex items-baseline gap-2 overflow-hidden">
              ...
              >
                {priceStr} {t('checkout.currency')}{product.unit ? ` / ${product.unit}` : ''}
              </span>
            </div>
          </div>
          ...
          <div className="text-[10px] sm:text-xs text-muted-foreground truncate">
            {product.shop?.name ?? t('checkoutSuccess.shopDefault')}
          </div>
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
            {(product.reviewsCount ?? 0) > 0 && product.avgRating != null ? (
              <>
                ...
                <span>·</span>
                <span>{product.reviewsCount === 1 ? t('product.reviewsOne') : t('product.reviewsMany', { count: product.reviewsCount ?? 0 })}</span>
              </>
            ) : (
              <span>{t('product.reviewsNone')}</span>
            )}
          </div>
        </div>
      </Link>
