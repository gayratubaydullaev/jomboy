...
export function ProductQuickView({ open, onOpenChange, productId, openVariantModalWhenReady, onVariantModalOpened }: ProductQuickViewProps) {
  ...
  const handleAddToCartClick = () => {
    if (hasOptions) {
      ...
      if (!variantId) {
        toast.error(t('product.pickVariant'));
        return;
      }
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        >
          <DialogTitle className="sr-only">
            {t('productCard.quickViewDialogTitle', { title: product?.title ?? t('productCard.productFallback') })}
          </DialogTitle>
          <DialogDescription className="sr-only">{t('productCard.quickViewDialogDesc')}</DialogDescription>
            ...
            <div className="flex flex-col md:flex-row h-full overflow-y-auto md:overflow-hidden">
                      ...
                      {hasMultipleImages && (
                        <>
                          <button
                            ...
                            aria-label={t('productCard.imagePrevAria')}
                          >
                          ...
                          <button
                            ...
                            aria-label={t('productCard.imageNextAria')}
                          >
                          ...
                          <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                            {images.map((_, i) => (
                              <button
                                ...
                                aria-label={t('productCard.imageDotAria', { n: i + 1 })}
                              />
                            ))}
                          </div>
              ...
              <div className="w-full md:w-1/2 flex flex-col md:h-full md:overflow-y-auto bg-card">
                <div className="flex-1 space-y-6 p-4 md:p-6">
                      ...
                      <span className="text-muted-foreground">
                        {(product.reviewsCount ?? 0) > 0
                          ? product.reviewsCount === 1
                            ? t('product.reviewsOne')
                            : t('product.reviewsMany', { count: product.reviewsCount ?? 0 })
                  ...
                  {hasOptions && (
                    <div>
                      <h3 className="text-sm font-medium text-muted-foreground mb-2">{t('product.pickVariant')}</h3>
                      <ProductOptions
                        key={product.id}
                  ...
                  {product.shop && (
                    <p className="text-sm text-muted-foreground">
                      {t('product.shopLine')}{' '}
                      <span className="font-medium text-foreground">{product.shop.name ?? product.shop.slug}</span>
                    </p>
                  )}
                </div>
      ...
      <Dialog open={variantModalOpen} onOpenChange={setVariantModalOpen}>
        <DialogContent className="sm:max-w-md w-[90%] rounded-2xl" aria-describedby={undefined}>
          <DialogHeader>
            <DialogTitle>{t('product.pickVariant')}</DialogTitle>
            <DialogDescription className="sr-only">{t('productCard.variantModalSrDesc')}</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            {product?.options && (
              <ProductOptions
          ...
          <DialogFooter>
            <Button
              onClick={() => {
                ...
                else toast.error(t('product.pickVariant'));
              }}
              disabled={adding}
